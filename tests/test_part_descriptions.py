...
class TestPartDescription(unittest.TestCase):
    ...
    def test_create_part_description(self): # Test create part description with valid payload
    ...
    def test_get_all_part_descriptions(self): # Test retrieve all part descriptions with pagination
    ...
    def test_get_part_descriptions_by_id(self): # Test retrieve part descriptions by ID
        
        response = self.client.get('part_descriptions/1')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json['name'], self.payLoad['name'])
    
    def test_get_part_descriptions_invalid_id(self): # Test retrieve part descriptions by invalid ID
        
        response = self.client.get('part_descriptions/9999')
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.json['message'], "Invalid part description")

    def test_update_part_descriptions(self): # Test updating an existing part description
    ...
    def test_update_invalid_part_descriptions(self): # Test updating part description does not exist
    ...
    def test_delete__part_descriptions(self): # Test deleting existing part description 
    ...
    def test_delete_invalid_part_descriptions(self): # Test deleting part description does not exist
        
        response = self.client.delete('part_descriptions/999')
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.json['message'], "Part description not found")
    
    def test_search_part_description_by_name(self): # Test searching a part description by name 
        
        response = self.client.get('/part_descriptions/search', query_string={"name": self.payLoad['name']})
        self.assertEqual(response.status_code, 200)
        self.assertIsInstance(response.json, list)
        
    def test_search_part_description_by_brand(self): # Test searching a part description by brand 
        
        response = self.client.get('/part_descriptions/search', query_string={"brand": self.payLoad['brand']})
        self.assertEqual(response.status_code, 200)
        self.assertIsInstance(response.json, list)

    def test_search_part_description_no_params(self): # Test searching a part description with no parameters
